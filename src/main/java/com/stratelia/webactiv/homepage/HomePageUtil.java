...
public class HomePageUtil {
  ...
  public static void traceException(Throwable exception) {
    ...
    if ((lastEx != null) && (lastEx.getMessage() != null) &&
        (lastEx.getMessage().indexOf("Connection reset by peer: socket write error") >= 0) &&
        (lastEx.getMessage().indexOf("SQL") < 0)) {
      SilverTrace.info("util", "HomePageUtil.traceException()", "root.EX_IGNORED", "Deepest",
          lastEx);
      SilverTrace.info("util", "HomePageUtil.traceException()", "root.EX_IGNORED", "Highest",
          exception);
    } else {
