...
public class SpaceChoiceTag extends TagSupport 
{
	...
	public int doStartTag() throws JspException
    {
        try
		{
			...
			pageContext.getOut().println("		<img src='icons/1px.gif' height='20' width='0' align='absmiddle'>");
			pageContext.getOut().println("		<span class='selectNS'>");
			pageContext.getOut().println("			<select name='privateDomain' size=1 onChange=changeSpace('" + m_sContext + "/admin/jsp/Main.jsp')>");
			...
			for(int nI = 0; nI < m_asPrivateDomainsIds.length; nI++)
			{
							   ...
							   if (spaceInst.getDomainFatherId().equals("0"))
							   {
									  ...
									  {
											  pageContext.getOut().println("				<option selected value=" + m_sPrivateDomain + ">" + shortDomain(_sPrivateDomainName, m_nMax) + "</option>");
									  }
									  else
											  pageContext.getOut().println("				<option value=" + m_asPrivateDomainsIds[nI] + ">" + shortDomain(_sPrivateDomainName, m_nMax) + "</option>");
							   }
			}

			pageContext.getOut().println("			</select>");
			pageContext.getOut().println("		</span>");
			pageContext.getOut().println("		<a href='#' onclick=changeSpace('" + m_sContext + "/admin/jsp/Main.jsp')><img src='icons/1px.gif' width='2' height='1' border='0'><img src='" + m_homeSpaceIcon + "' border='0' align='absmiddle' alt='" + m_messageBackToMainPage + "' title='" + m_messageBackToMainPage + "'></a>");
