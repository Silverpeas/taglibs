...
public class GalleryTag extends TagSupport {
  ...
  public int doStartTag() throws JspTagException {
    ...
    GalleryTagUtil galleryTagUtil = (GalleryTagUtil) pageContext.getAttribute(
        getName(), iScope);
    if (galleryTagUtil == null || "true".equals(forceReload)) {
      String userId = AuthenticationManager.getUserId(
          (HttpServletRequest) pageContext.getRequest());

      // create a new object which has a reference on Notation EJB
      galleryTagUtil = new GalleryTagUtil(getComponentId(), getElementId(), userId);
      pageContext.setAttribute(getName(), galleryTagUtil, iScope);
    }
    return EVAL_PAGE;
  }
