...
public class CommentTag extends TagSupport {
  ...
  public int doStartTag() throws JspTagException {
    ...
    CommentTagUtil commentTagUtil = (CommentTagUtil) pageContext.getAttribute(
        getName(), iScope);
    if (commentTagUtil == null || "true".equals(forceReload)) {
      String userId = AuthenticationManager.getUserId(
          (HttpServletRequest) pageContext.getRequest());

      // create a new object which has a reference on Notation EJB
      commentTagUtil = new CommentTagUtil(getComponentId(), getElementId(), userId);
      pageContext.setAttribute(getName(), commentTagUtil, iScope);
    }
    return EVAL_PAGE;
  }
