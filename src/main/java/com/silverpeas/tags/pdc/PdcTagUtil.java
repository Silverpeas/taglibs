...
public class PdcTagUtil implements java.io.Serializable {
  ...
  private String getUserId() {
    if (userId == null) {
      SilverTrace.info("Pdc", "PdcTagUtil.getUserId()", "root.MSG_GEN_PARAM_VALUE", "userId = "
          + SiteTagUtil.getUserId());
  ...
  public Value getAltValue(String axisIdAndValueIdAndVocaId) throws Exception {
    ...
    SilverTrace.info("Pdc", "PdcTagUtil.getAltValue()", "root.MSG_GEN_PARAM_VALUE", "axisId = "
        + axisId + ", valueId = " + valueId + ", vocaId = " + vocaId);

    Value value = getValue(axisId, valueId);

    SilverTrace.info("Pdc", "PdcTagUtil.getAltValue()", "root.MSG_GEN_PARAM_VALUE", "value = "
        + value.toString());
    ...
    if (synonym != null) {
      SilverTrace.info("Pdc", "PdcTagUtil.getAltValue()", "root.MSG_GEN_PARAM_VALUE", "synonym = "
          + synonym.toString());
  ...
  /**
   * Remplace le nom des valeurs d'un axe par leurs synonymes. La valeur reste inchangée si elle n'a
   * pas de synonyme.
   *
  ...
  private List getSynonymsOfValue(String valueId, String vocaId) throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getSynonymsOfValue()", "root.MSG_GEN_PARAM_VALUE",
        "valueId = " + valueId + ", vocaId = " + vocaId);
    long lvocaId = new Long(vocaId).longValue();
  ...
  public List getSubTreeView() throws Exception {
    return valuesTree2MenuItemTree((ArrayList) getPdcBm().getSubAxisValues(getAxisId(),
        getValueId()));
  }
   ...
   * @return une liste contenant les filles de la valeur sélectionnée ainsi que tous les ascendants
   * de la valeur et leurs soeurs.
   * @throws Exception
   */
  public List getSubTreeViewContextual(String targetValueId) throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getSubTreeViewContextual()", "root.MSG_GEN_PARAM_VALUE",
  ...
  public List getSilverContents() throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContents()", "root.MSG_GEN_PARAM_VALUE",
        "No Sorting !");
    List componentIds = getAvailableComponentIds();
    List globalSilverContents = getSilverContents(getValueId(), componentIds);
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContents()", "root.MSG_GEN_PARAM_VALUE",
        "globalSilverContents.size() = " + globalSilverContents.size());
  ...
  public List getOrderedSilverContents(String columnAndSort) throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getOrderedSilverContents()", "root.MSG_GEN_PARAM_VALUE",
        "columnAndSort = " + columnAndSort);
    ...
    SilverTrace.info("Pdc", "PdcTagUtil.getOrderedSilverContents()", "root.MSG_GEN_PARAM_VALUE",
        "comparator is null ? " + (comparator == null));

    Collections.sort(silverContents, comparator);

    SilverTrace.info("Pdc", "PdcTagUtil.getOrderedSilverContents()", "root.MSG_GEN_PARAM_VALUE",
        "silverContents sorted. Size = " + silverContents.size());
  ...
  public List getSilverContentsRecursiveGroupByValues(String axisIdAndColumnAndSort)
      throws Exception {
    String axisId = axisIdAndColumnAndSort.substring(0, axisIdAndColumnAndSort.indexOf(","));
    String columnAndSort =
        axisIdAndColumnAndSort.substring(axisIdAndColumnAndSort.indexOf(",") + 1,
        axisIdAndColumnAndSort.length());
  ...
  private List getSilverContentsGroupByValues(String axisId, String columnAndSort,
      boolean recursiveMode) throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
    ...
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
        "root.MSG_GEN_PARAM_VALUE", "silverContentsToReturn.size() = " + silverContentsToReturn.
        size());
    ...
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
        "root.MSG_GEN_PARAM_VALUE", "allSilverContents.size() = " + allSilverContents.size());
    ...
    for (int i = 0; i < allSilverContents.size(); i++) {
      ...
      if (item.getType() == MenuItem.TYPE_PDC_VALUE) {
        ...
        SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
            "root.MSG_GEN_PARAM_VALUE", "value added = " + item.getName());
      } else {
        indexInList = isInList(item.getId(), silverContentsToReturn);
        if (indexInList == -1) {
          // The silverContent is not in the initial list.
          SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
              "root.MSG_GEN_PARAM_VALUE", "silvercontent is not in the initial list : " + item.
              getName());
        } else {
          ...
          SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
              "root.MSG_GEN_PARAM_VALUE", "silvercontent is in the initial list : " + item.getName());
    ...
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
        "root.MSG_GEN_PARAM_VALUE", "start to remove empty values");
    ...
    if (silverContentsToReturn.size() > 0) {
      SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
          "root.MSG_GEN_PARAM_VALUE", "start to add items which are not in second axis");
    ...
    SilverTrace.info("Pdc", "PdcTagUtil.getSilverContentsGroupByValues()",
        "root.MSG_GEN_PARAM_VALUE", "result returned !");
  ...
  public Collection getValuesOnAxis(String objectIdAndComponentIdAndAxisId) throws Exception {
    String axisIdStr =
        objectIdAndComponentIdAndAxisId.substring(
        objectIdAndComponentIdAndAxisId.lastIndexOf(",") + 1, objectIdAndComponentIdAndAxisId
        .length());

    SilverTrace.info("Pdc", "PdcTagUtil.getValuesOnAxis()", "root.MSG_GEN_PARAM_VALUE",
        "axisIdStr = " + axisIdStr);
    ...
    for (int i = 0; i < positions.size(); i++) {
      ...
      SilverTrace.info("Pdc", "PdcTagUtil.getValuesOnAxis()", "root.MSG_GEN_PARAM_VALUE",
          "classifyValues.size() = " + classifyValues.size());

      ClassifyValue classifyValue = null;
      int size = -1;
      for (int j = 0; j < classifyValues.size(); j++) {
        SilverTrace.info("Pdc", "PdcTagUtil.getValuesOnAxis()", "root.MSG_GEN_PARAM_VALUE", "j = "
            + j);
        classifyValue = (ClassifyValue) classifyValues.get(j);
        SilverTrace.info("Pdc", "PdcTagUtil.getValuesOnAxis()", "root.MSG_GEN_PARAM_VALUE",
            "classifyValue = " + classifyValue.toString());
  ...
  public String getClassifiedOnAxisValue(String objectIdAndComponentIdAndAxisIdAndValueId)
      throws Exception {
    String result = "false";

    String valueId =
        objectIdAndComponentIdAndAxisIdAndValueId.substring(
        objectIdAndComponentIdAndAxisIdAndValueId.lastIndexOf(",") + 1,
        objectIdAndComponentIdAndAxisIdAndValueId.length());

    SilverTrace.info("Pdc", "PdcTagUtil.isClassifiedOnAxisValue()", "root.MSG_GEN_PARAM_VALUE",
        "valueId = " + valueId);

    ArrayList values =
        (ArrayList) getValuesOnAxis(objectIdAndComponentIdAndAxisIdAndValueId.substring(0,
        objectIdAndComponentIdAndAxisIdAndValueId.lastIndexOf(",")));
  ...
  private List getSilverContents(String valueId, List componentIds, boolean recursive)
      throws Exception {
    SearchContext context = getSearchContext(getAxisId(), valueId);
    return findSilverContents(context, componentIds, recursive);
  }

  private List findSilverContents(SearchContext context, List componentIds, boolean recursiveSearch)
      throws Exception {
    ...
    if (SiteTagUtil.isDevMode() || SiteTagUtil.isRecetteMode()) {
      // Le site n'est pas en mode production. Une recherche est faite sur le PDC sans tenir compte
      // de la colonne isVisible.
      visibilitySensitive = false;
    } else {
      // Le site est en mode production. Une recherche est faite sur le PDC en tenant compte de la
      // colonne isVisible.
      // C'est le comportement standard de Silverpeas.
    }

    List silverContents = new ArrayList();
    List silverContentsToFiltered =
        getPdcBm().findGlobalSilverContents(context, componentIds, recursiveSearch,
        visibilitySensitive);

    SilverTrace.info("Pdc", "PdcTagUtil.findSilverContents()", "root.MSG_GEN_PARAM_VALUE",
        "silverContentsToFiltered = " + silverContentsToFiltered.size());
    ...
    if (SiteTagUtil.isDevMode()) {
      ...
      for (int i = 0; i < silverContentsToFiltered.size(); i++) {
        ...
        if (getPublicationTagUtil().isPublicationVisible(
            new PublicationPK(gsc.getId(), "useless", gsc.getInstanceId()))) {
  ...
  private List getAvailableComponentIds() throws Exception {
    ...
    if (getComponentId() != null) {
      ...
      for (int i = 0; i < allComponents.size(); i++) {
        ...
        if (instanceId != null && (instanceId.startsWith("kmelia") || instanceId.startsWith(
            "quickInfo") || instanceId
            .startsWith("almanach"))) {
          instanceIds.add(instanceId);
          SilverTrace.info("Pdc", "PdcTagUtil.getAvailableComponentIds()",
  ...
  private SearchContext getSearchContext(String axisId, String valueId) throws Exception {
    ...
    if (valueId != null && !valueId.startsWith("/")) {
      SilverTrace.info("Pdc", "PdcTagUtil.getSearchContext()", "root.MSG_GEN_PARAM_VALUE",
          "axisId = " + axisId + ", valueId = " + valueId + " isn't the path !");
  ...
  /*
   * Get a tree containing values and silverContents according to each value. Values that not
   * contains any silverContents are not returned.
   * @return a sorted list of MenuItem
   */
  private ArrayList getFullTreeView(String axisId, String valueId, String columnAndSort)
      throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getFullTreeView()", "root.MSG_GEN_PARAM_VALUE",
        "axisId = " + axisId + ", valueId = " + valueId + ", columnAndSort = " + columnAndSort);
    ...
    for (int i = 0; i < reversedTreeView.size(); i++) {
      ...
      SilverTrace.info("Pdc", "PdcTagUtil.getFullTreeView()", "root.MSG_GEN_PARAM_VALUE",
          "silverContents.size() = " + silverContents.size());

      if (silverContents.size() > 0) {
        if (columnAndSort != null) {
          // we have to sort silverContents
          SilverTrace.info("Pdc", "PdcTagUtil.getFullTreeView()", "root.MSG_GEN_PARAM_VALUE",
              "ordering silvercontents on value " + value.getValuePK().getId());
          Collections.sort(silverContents, comparator);
        }
        ...
        if (lastValueAdded != null && value.getValuePK().getId().
            equals(lastValueAdded.getMotherId())) {
          fullTreeView.add(value2MenuItem(value));
          lastValueAdded = value;
        }
      }
      ...
      SilverTrace.info("Pdc", "PdcTagUtil.getFullTreeView()", "root.MSG_GEN_PARAM_VALUE",
          "fullTreeView.size() = " + fullTreeView.size());
    }
  ...
  private ArrayList addSilverContentsToTreeView(List silverContents, ArrayList tree, Value value)
      throws Exception {
    ...
    for (int j = 0; j < silverContents.size(); j++) {
      ...
      SilverTrace.info("Pdc", "PdcTagUtil.addSilverContentsToTreeView()",
          "root.MSG_GEN_PARAM_VALUE", "new item added = " + item.getName());
    }
    return tree;
  }
  ...
  private MenuItem value2MenuItem(Value value) {
    MenuItem item =
        new MenuItem(value.getName(), value.getDescription(), value.getLevelNumber(),
        MenuItem.TYPE_PDC_VALUE, value.getPK().getId(), value.getFullPath());
    return item;
  }
  ...
  private MenuItem silverContent2MenuItem(GlobalSilverContent gsc) throws Exception {
    MenuItem item =
        new MenuItem(gsc.getName(), gsc.getDescription(), 0, MenuItem.TYPE_COMPONENT_CONTENT, gsc
        .getId(), "unknown");
  ...
  private MenuItem silverContent2MenuItem(GlobalSilverContent gsc, Value value) throws Exception {
    MenuItem item =
        new MenuItem(gsc.getName(), gsc.getDescription(), value.getLevelNumber() + 1,
        MenuItem.TYPE_COMPONENT_CONTENT, gsc.getId(), value.getFullPath());
    item.setComponentId(gsc.getInstanceId());
  ...
  private GlobalSilverContent addSpaceId2GlobalSilverContent(GlobalSilverContent gsc)
      throws Exception {
    if (!isSpaceIdSkipped()) {
  ...
  private String getSpaceId(String instanceId) throws Exception {
    SilverTrace.info("Pdc", "PdcTagUtil.getSpaceId()", "root.MSG_GEN_PARAM_VALUE",
        "instanceId a = " + instanceId);
    ComponentInstLight compoInst = getAdminBm().getComponentInstLight(instanceId);
    SilverTrace.info("Pdc", "PdcTagUtil.getSpaceId()", "root.MSG_GEN_PARAM_VALUE", "spaceId = "
        + compoInst.getDomainFatherId());
    return compoInst.getDomainFatherId();
  }

  private Comparator getComparator(String columnAndSort) {
    SilverTrace.info("Pdc", "PdcTagUtil.getComparator()", "root.MSG_GEN_PARAM_VALUE",
        "columnAndSort = " + columnAndSort);
    if (columnAndSort != null) {
      int i = 1;
      String column = "";
      ...
      SilverTrace.info("Pdc", "PdcTagUtil.getComparator()", "root.MSG_GEN_PARAM_VALUE",
          "column = " + column);
      SilverTrace.info("Pdc", "PdcTagUtil.getComparator()", "root.MSG_GEN_PARAM_VALUE", "sort = "
          + sort);
      ...
      } else if (column != null && (column.equalsIgnoreCase("Date") || column.equalsIgnoreCase(
          "UpdateDate"))) {
        if (sort != null && sort.equalsIgnoreCase("Desc")) {
  ...
  private PdcBm getPdcBm() {
    if (pdcBm == null) {
      try {
        ...
        throw new PdcBmRuntimeException("PdcTagUtil.getPdcBm", SilverpeasRuntimeException.ERROR,
  ...
  private AdminBusiness getAdminBm() {
    if (adminBm == null) {
      try {
        ...
        throw new PublicationRuntimeException("PdcTagUtil.getAdminBm",
  ...
  private ThesaurusBm getThesaurusBm() {
    if (thesaurusBm == null) {
      try {
        thesaurusBm = EJBUtilitaire.
            getEJBObjectRef(JNDINames.THESAURUSBM_EJBHOME, ThesaurusBm.class);
      } catch (Exception e) {
        throw new PublicationRuntimeException("PdcTagUtil.getThesaurusBm",
            SilverpeasRuntimeException.ERROR, "root.EX_CANT_GET_REMOTE_OBJECT", e);
      }
    }
    return thesaurusBm;
  ...
  public GlobalSilverContent getTranslatedGlobalSilveContent(GlobalSilverContent gsc,
    ...
    if (StringUtil.isDefined(lang)) {
      GlobalSilverContentI18N gsci18n =
          (GlobalSilverContentI18N) gsc.getTranslation(getSiteLanguage());
      if (gsci18n != null) {
